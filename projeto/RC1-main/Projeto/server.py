...
def handle_client_connection(client_socket,address):
    print('Status de {}:{}'.format(address[0], address[1]))
    headerformat='BLL'
    try:
        while True:
            ...
            request = client_socket.recv(size*2)
            pktdata, mem = struct.unpack('ff', request)
            
            print(f'CPU {round(pktdata,4)} MEM {round(mem, 4)}', end='\r')
